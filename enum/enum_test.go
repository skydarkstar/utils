...
func TestMember_Value(t *testing.T) {
	...
	is.Equal(Member[string]{"blue"}.Value, "blue")
	is.Equal(Member[int]{14}.Value, 14)
}
...
func TestEnum_Empty(t *testing.T) {
	...
	is.True(New[int, Member[int]]().Empty())
}

func TestEnum_Len(t *testing.T) {
	is := is.New(t)
	is.Equal(Colors.Len(), 3)
	is.Equal(New[int, Member[int]]().Len(), 0)
}
...
func TestBuilder(t *testing.T) {
	is := is.New(t)
	type Country Member[string]
	var (
		b         = NewBuilder[string, Country]()
